"""
Module 4 — config_manager

Manages the loading of sensitive configuration for the CSV Data Analyst
Agent. Keeps all credential-handling logic in one place so that the rest
of the system never touches environment variables or .env files directly.

Responsibilities:
    - Load environment variables from a .env file using python-dotenv.
    - Provide access to the ANTHROPIC_API_KEY environment variable.
    - Validate the presence of the ANTHROPIC_API_KEY at startup.

Security notes:
    - The .env file MUST be excluded from version control via .gitignore.
    - The API key is never logged, printed, or stored on disk outside .env.
    - A .env.example template is provided in the repository for new users.
"""
